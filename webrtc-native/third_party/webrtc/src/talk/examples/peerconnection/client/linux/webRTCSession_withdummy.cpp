...
//	pco = new PCObserver();
	...
	LOG(INFO) << "webRTCSession::webRTCSession" << endl;
       ...
       printf(" webRTCSession::webRTCSession pc_factory =%p ,this =%p\n",(void *)pc_factory,this);
       printf(" webRTCSession::webRTCSession pc_factory =%p ,this =%p \n",(void *)&pc_factory,this);
	setDefaultOptions();
}
...
void webRTCSession::on(SessionObserver *lSessionObserver) {
	this->lSessionObserver = lSessionObserver;
}
...
void webRTCSession::startSession(webrtc::PeerConnectionInterface::IceServers ice_servers) {
	...
	const std::string peerConnectionId = boost::lexical_cast<std::string>(
			tPeerConnectionId);
	cout << "create factory value!!!i11111"<< endl;
	 printf(" webRTCSession::startSession pc_factory this= %p \n",this);
	 printf(" webRTCSession::startSession pc_factory = %p \n",(void *)&pc_factory);
	printf("1000000000000");
 printf(" webRTCSession::startSession pc_factory =%p this =%p \n",(void *)(pc_factory),this);	
 printf(" webRTCSession::startSession pc_factory =%p \n",(void *)&pc_factory,this);	
	pc_factory.release();
cout << "create factory 2 !!!" << endl;
	 ...
	 rtc::scoped_refptr<PeerConnectionFactoryInterface> pc_factory = webrtc::CreatePeerConnectionFactory();//Shayo
...
//	 &constraints,
	...
	m_pc_ = pc_factory->CreatePeerConnection(ice_servers,NULL, NULL, NULL, this);
...
//	pc_->CreateOffer(dummyconductor, NULL);
...
void webRTCSession::DummyOnIceGatheringChange(
